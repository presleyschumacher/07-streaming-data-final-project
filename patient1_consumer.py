"""
    ...
    The troponin_producer.py must run to start sending the message first
    We want know if (Condition To monitor):
        The patient's troponin levels have increased by 7ng/l or more in the last hour AND/OR
        The patient's troponin levels are at 30ng/l or higher
...
def callback(ch, method, properties, body):
    ...
    try:
        ...
        # If the condition is met, print a message indicating that the troponin level has increased by 7 or more in the last hour
        ...
        # Add a second alert for when the troponin level is at 30 or greater
    ...
    # The message won't be removed from the queue when:
    # The index error occurs when the message being processed doesn't contain a troponin level value
    # The value error occurs when the troponin level in the message is not a valid float
    except IndexError:
...
def main(hn: str = "localhost", qn: str = "patient1"):
